import * as rpc from './ipc_tauri'
import { Event, IPCError, Listener, ListenersWithCriteria } from './types'
import { DropdownOptionJSON, FilterCriteria, JSONCommand, JSONResponse } from './rpc'
...
function getMessageId() {
	const msgId = Math.floor(Math.random() * maxMessageId)
	return msgId
}

type NarrowUnion<T extends {}, Key extends keyof T, Value extends T[Key]> =
	T extends Record<Key, Value> ? T : never

type RPCCommand<T extends JSONCommand['type']> = Omit<
	NarrowUnion<JSONCommand, 'type', T>,
	'message_id'
>
type RPCResponse<T extends JSONResponse['type']> = NarrowUnion<JSONResponse, 'type', T>

function sendRPC<T extends RPCCommand<'CallService'>>(
	message: T
): Promise<RPCResponse<'CallServiceResponse' | 'CallServiceError'>>
function sendRPC<T extends RPCCommand<'GetServices'>>(
	message: T
): Promise<RPCResponse<'ServiceList'>>
function sendRPC<T extends RPCCommand<'GetOptions'>>(
	message: T
): Promise<RPCResponse<'OptionsList'>>
...
function sendRPC<T extends Omit<JSONCommand, 'message_id'>>(message: T) {
	...
	case 'GetOptions':
		return new Promise((res, rej) => {
			...
			rpc.sendRPC({ ...(message as any), message_id }).catch((reason) => {
...
rpc.connect((event) => {
	...
	case 'OptionsList':
		...
		break
	case 'Event':
		const listeners = ipcEvents.get(event.name)
...
export async function listen<T = unknown>(
	...
	async function subscribe() {
		try {
			...
			if (
				listeners.none.size === 0 &&
				listeners.uuid.size === 0 &&
				listeners.string.size === 0
			) {
	...
	case 'Uuid':
		...
		const uuidListeners = listeners.uuid.get(criteria.data)!
	...
	case 'String':
		...
		const stringListeners = listeners.string.get(criteria.data)!
...
export type MatchArms<Enum extends RustEnum, ReturnType> = UnionToIntersection<
	Enum extends string
		? { [K in Enum]: () => ReturnType }
		: {
				[K in keyof Enum]: ((arg: Enum[K]) => ReturnType) | null
			}
>
export type MatchArmsOriginal<Enum extends RustEnum, ReturnType> = UnionToIntersection<
	Enum extends string
		? { [K in Enum]: () => ReturnType }
		: {
...
export type MatchReturn<
	Enum extends RustEnum,
	Cases extends MatchArms<Enum, any> | MatchArmsOriginal<Enum, any>
> = Cases extends MatchArms<Enum, infer ReturnType> ? ReturnType : never
 ...
 * a template.
 */
export type ExhaustiveFlag<
	_Enum extends RustEnum,
	_HandledCases extends { [K in keyof UnionToIntersection<_Enum>]: any }
> = never
 ...
 */
export function exhaustiveMatch<Enum extends RustEnum, Cases extends MatchArms<Enum, any>>(
	rustEnum: Enum,
	arms: Cases
): MatchReturn<Enum, Cases> {
	if (typeof rustEnum === 'string') return (arms as any)[rustEnum]()
	const entries = Object.entries(rustEnum)
	if (entries.length !== 1)
		throw new Error('Invalid Rust enum. This should have been checked at compile time.')
 ...
 */
export function exhaustiveMatchOriginal<
	Enum extends RustEnum,
	Cases extends MatchArmsOriginal<Enum, any>
>(rustEnum: Enum, arms: Cases): MatchReturn<Enum, Cases> {
	if (typeof rustEnum === 'string') return (arms as any)[rustEnum]()
