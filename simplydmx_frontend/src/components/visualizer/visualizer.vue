...
watch(
	() => [props.displayData, ...fixtures.keys()],
	() => {
...
function getActiveControlGroups(fixtureId: string): ControlGroup[] | null {
	...
	if (!patcherState.value || !fixtures.has(fixtureId) || !props.displayData) return null
	...
	const activeControlGroups = fixtureTypeInfo.control_groups.filter((group) =>
		exhaustiveMatch(group.channels, {
			RGBGroup: ({ red, green, blue }) => includesAll(availableChannels, [red, green, blue]),
			CMYKGroup: ({ cyan, magenta, yellow, black }) =>
				includesAll(availableChannels, [cyan, magenta, yellow, black]),
			...
			PanTilt: ({ pan, tilt }) => includesAll(availableChannels, [pan, tilt]),
...
function getDefaultFixtureValues(
	fixtureProfile: FixtureInfo,
...
async function updateLight(fixtureId: string) {
	...
	// Collect data and drop unsynchronized triggers
	if (!patcherState.value || !fixtures.has(fixtureId) || !props.displayData) return
	...
	let fixtureData =
		props.displayData[fixtureId] ||
		getDefaultFixtureValues(fixtureTypeInfo, fixturePersonalityInfo)
	let fixtureVis = fixtures.get(fixtureId)
	...
	if (controlGroups)
		controlGroups.forEach((group) =>
			exhaustiveMatch(group.channels, {
				...
				Gobo: (_gobo) => {},
				Intensity: (intensity) => {
					intensityValue = normalizeChannel(
						fixtureTypeInfo,
						fixtureData,
						intensity,
						'percentage'
...
const activeControlGroups = computed(() => {
			...
			if (group.name === null) {
				...
				exhaustiveMatchOriginal(group.channels, {
					Intensity: (controlData) =>
						intensity.controls.push({
							instanceId: fixtureId!,
							controlData
						}),
					RGBGroup: (controlData) =>
						colorGroup.controls.push({
							instanceId: fixtureId!,
							controlData
						}),
	...
	;[intensity, colorGroup, position, zoom].forEach(
		(group) => group.controls.length && groups.push(group)
	)

	// Add unknown groups
	Object.keys(other)
		.sort()
		.forEach((key) => groups.push(other[key]))
	...
	nextTick(() => {
		...
		vis.value.on('selection:updated', (event) => {
			...
			if (event.deselected.length)
				selected.value = selected.value.filter((obj) => {
...
function handleBatchedProps() {
	if (batchedData === null) {
		...
		batchedData = null
		props.updateProps(myBatchedData).then(() => handleBatchedProps())
	}
}
...
function handleUpdatedProps(data: SubmasterData) {
	if (pushInProgress) {
		...
		pushInProgress = true
		props.updateProps(data).then(() => handleBatchedProps())
	}
}
</script>
