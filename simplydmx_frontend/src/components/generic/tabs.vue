...
const activeTabElementBounds = useElementBounding(
	() => tabElements.value[props.tabs!.findIndex((tab) => tab.id === activeTabId.value)]
)
const activeLineWidth = computed(() => activeTabElementBounds.width.value)
const activeLineOffset = computed(
	() => (activeTabElementBounds.left.value || 0) - (tabsContainerBounds.left.value || 0)
)
</script>
