...
const deletingUniverseName = computed(() => {
	...
	return (
		universeOptions.value?.find((item) => item.value === deleteUniverseConfirm.value)?.name ||
		null
	)
})
...
<template>
	<div class="patcher-universes">
		...
		<Dialog :visible="deleteUniverseConfirm != null">
			<template #header> Delete universe? </template>
			Are you sure you want to delete the universe &quot;{{
				deletingUniverseName
			}}&quot;?<br /><br />
			<span style="opacity: 0.75"
				>Any fixtures currently associated with this universe will be unlinked.</span
			>
			<template #footer>
				<Button @click="deleteUniverseConfirm = null" class="spaced"> No </Button>
				<div class="spacer" />
				<Button @click="deleteQueuedUniverse()" class="spaced"> Yes </Button>
			</template>
		</Dialog>
	</div>
</template>
