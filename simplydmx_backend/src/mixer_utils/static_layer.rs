...
use super::{
	...
	layer::MixerLayer,
	state::{FullMixerOutput, SubmasterData, LightInstanceId, LightAttributeId},
};
...
impl MixerLayer for StaticLayer {
	...
	async fn cleanup(&mut self, patcher_data: &(FullMixerOutput, FullMixerBlendingData)) {
		...
		let fixture_keys: Vec<LightInstanceId> = self.values.keys().cloned().collect();
		for fixture_id in fixture_keys {
			if let Some(fixture_base) = patcher_data.0.get(&fixture_id) {
				...
				let attribute_keys: Vec<LightAttributeId> = fixture_data.keys().cloned().collect();
