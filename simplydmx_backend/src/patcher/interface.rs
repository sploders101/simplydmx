use super::{
	driver_plugin_api::SharableStateWrapper,
	fixture_types::{ChannelSize, ChannelType, Segment, FixtureInfo},
	state::{PatcherContext, VisualizationInfo, FixtureInstance},
};
use crate::{
	mixer_utils::state::{BlendingData, FullMixerBlendingData, FullMixerOutput, SnapData},
	utils::forms::FormDescriptor,
};
...
impl PatcherInterface {
	...
	pub async fn import_fixture(
		...
		// if let Some(output_driver) = ctx.output_drivers.get(&fixture_info.output_driver) {
		// 	if let Err(controller_error) = output_driver
		// 		.import_fixture(&fixture_info.id, output_info)
		// 		.await
		// 	{
		...
		// 		self.0
		// 			.emit_if_needed("patcher.new_fixture".into(), FilterCriteria::None, || {
		// 				fixture_info.clone()
		// 			})
		// 			.await;
		// 		ctx.sharable
		// 			.library
		// 			.insert(fixture_info.id.clone(), fixture_info);
		// 		return Ok(());
		// 	}
		// } else {
		...
		if let Some(fixture_type_info) = ctx.sharable.library.get(&fixture_type) {
			if let Some(controller) = ctx.output_drivers.get(&fixture_type_info.output_driver) {
					...
					.await
				{
					...
					ctx.sharable.fixtures.insert(instance_uuid.clone(), fixture);
	...
	pub async fn delete_fixture(&self, fixture_id: &Uuid) -> Result<(), DeleteFixtureError> {
		...
		if let Err(controller_err) = controller.remove_fixture_instance(fixture_id).await {
			...
			ctx.sharable.fixtures.insert(instance_id, fixture);
			return Err(DeleteFixtureError::ErrorFromController(
				controller_err.to_string(),
			));
		} else {
			ctx.sharable
				.fixture_order
				.retain(|fixture| fixture != fixture_id);
			self.0
				.emit(
					"patcher.patch_updated".into(),
					FilterCriteria::Uuid(fixture_id.clone()),
					Option::<FixtureInstance>::None,
				)
				.await;
			return Ok(());
		}
	...
	pub async fn edit_fixture(
			...
			.await
		{
			...
			self.0
				...
				.await;
			ctx.sharable.fixtures.insert(fixture.id, new_instance);
			return Ok(());
		}
	}
	...
	fn apply_virtual_intensities(
		...
		for (fixture_id, fixture_values) in data {
			...
			let fixture_type =
				unwrap_continue!(ctx.sharable.library.get(&fixture_instance.fixture_id));
...
pub enum EditFixtureError {
	...
	FixtureMissing,
	#[error(
		"[Internal state error]: The fixture definition is missing for the requested fixture type"
	)]
	FixtureTypeMissing,
...
#[portable]
pub enum FixtureOutputData {
}

#[portable]
pub enum FixtureEditData {
}
