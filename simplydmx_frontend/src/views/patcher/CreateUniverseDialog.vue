...
watch(
	...
	(queryDriver) => {
		if (queryDriver) {
			output_dmx.get_link_universe_form(queryDriver, null).then((formResult) => {
...
<template>
	<Dialog :visible="props.visible" :show-close="false">
		<template #header> Add Universe </template>
		...
		<template #footer>
			<Button @click="cancelAddingUniverse()" subtle class="spaced"> Cancel </Button>
			<div class="spacer" />
			<Button @click="addUniverse()" class="spaced"> Ok </Button>
		</template>
	</Dialog>
