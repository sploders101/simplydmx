...
const deletingSubmasterName = computed(() => {
	...
	return (
		submasterOptions.value?.find((item) => item.value === deleteSubmasterConfirm.value)?.name ||
		null
	)
})
...
<template>
	<div class="submaster-editor">
		...
		>
			<template #header-right>
				<Tooltip text="Create Submaster">
					<Button @click="createSubmasterDialog = true" icon subtle
						><Icon i="plus"
					/></Button>
				</Tooltip>
			</template>
				...
				<div v-if="selectedSubmaster === option.value" class="rename-button">
					<Tooltip text="Rename submaster" placement="right">
						<Button
							...
							icon
							@click="
								renameSubmaster = option.value
								newSubmasterName = option.name
							"
						>
		...
		<div class="submaster-editor__visualizer">
			<Visualizer
				v-if="submasterData"
				:display-data="submasterData"
				:update-props="updateSubmaster"
			/>
		</div>

		<CreateSubmasterDialog v-model:visible="createSubmasterDialog" />

		<Dialog :visible="deleteSubmasterConfirm != null">
			<template #header> Delete submaster? </template>
			Are you sure you want to delete the submaster &quot;{{
				deletingSubmasterName
			}}&quot;?<br /><br />
			<span style="opacity: 0.75"
				>All data currently associated with this submaster will be deleted.</span
			>
			<template #footer>
				<Button @click="deleteSubmasterConfirm = null" class="spaced"> No </Button>
				<div class="spacer" />
				<Button @click="deleteQueuedSubmaster()" class="spaced"> Yes </Button>
			</template>
		</Dialog>

		<Dialog :visible="renameSubmaster != null">
			<template #header> Rename submaster </template>
			<Textbox v-model="newSubmasterName" />
			<template #footer>
				<Button @click="renameSubmaster = null" class="spaced"> Cancel </Button>
				<div class="spacer" />
				<Button @click="renameQueuedSubmaster()" class="spaced"> Confirm </Button>
			</template>
		</Dialog>
	</div>
</template>
