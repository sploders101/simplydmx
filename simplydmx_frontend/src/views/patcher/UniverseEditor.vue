...
import { exhaustiveMatch, FormDescriptor, output_dmx } from '@/scripts/api/ipc'
...
function updateUniverseForm(queryUniverse: string | null) {
	if (queryUniverse) {
		waitForUniverseState().then(() => {
			let universeInfo = universeState.value?.universe_list.find(
				([id]) => queryUniverse == id
			)
			if (!universeInfo) return // This will almost always resolve itself one tick later.
			name.value = universeInfo[1]
			output_dmx.get_linked_controller(queryUniverse).then((driverId) => {
...
function updateDriver(queryDriver: string | null) {
	nextTick().then(() => {
		...
		if (queryDriver) {
			output_dmx.get_link_universe_form(queryDriver, queryUniverse).then((formResult) => {
...
async function editUniverse() {
	if (name.value && addUniverseForm.value) {
		let universeInfo = universeState.value?.universe_list.find(
			([id]) => props.selectedUniverse == id
		)
		if (!universeInfo)
			return (creationError.value = {
		...
		if (name.value !== universeInfo[1]) {
			await output_dmx.rename_universe(props.selectedUniverse, name.value)
		}
		let response
...
<template>
	<Textbox label="Name" v-model="name" class="spaced" />
	...
	<div v-if="editsMade" class="universe-editor__actions">
		<Button @click="cancelEditingUniverse()" subtle class="spaced"> Cancel </Button>
		<div class="spacer" />
		<Button @click="editUniverse()" class="spaced"> Ok </Button>
	</div>
	<Dialog :visible="!!creationError" @close-clicked="creationError = null">
		<template #header>
		...
		<template #footer>
			<div class="spacer" />
			<Button @click="creationError = null" class="spaced"> Ok </Button>
		</template>
	</Dialog>
</template>
