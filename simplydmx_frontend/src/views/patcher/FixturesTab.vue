...
const deletingFixtureName = computed(() => {
	...
	return (
		fixtureOptions.value?.find((item) => item.value === deleteFixtureConfirm.value)?.name ||
		null
	)
})
...
<template>
	<div class="patcher-add">
		...
		<Dialog :visible="deleteFixtureConfirm != null">
			<template #header> Delete fixture? </template>
			Are you sure you want to delete the fixture &quot;{{
				deletingFixtureName
			}}&quot;?<br /><br />
			<span style="opacity: 0.75"
				>All data currently associated with this fixture will be deleted.</span
			>
			<template #footer>
				<Button @click="deleteFixtureConfirm = null" class="spaced"> No </Button>
				<div class="spacer" />
				<Button @click="deleteQueuedFixture()" class="spaced"> Yes </Button>
			</template>
		</Dialog>
	</div>
</template>
