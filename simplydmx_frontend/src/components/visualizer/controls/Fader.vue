...
const faderValue = computed(() => {
	...
	props.group.controls.forEach((control) => {
		const profile =
			patcherState.value?.library[
				patcherState.value!.fixtures[control.instanceId].fixture_id
			]!
...
function updateChannels(newValue: number) {
	...
	props.group.controls.forEach((control) => {
		...
		const normalizeValue = (channel: string) =>
			Math.floor(
				newValue *
					exhaustiveMatch(profile.channels[channel].size, {
						U8: () => 255,
						U16: () => 65535
					})
			)
...
<template>
	<Fader :label="props.group.name" :modelValue="faderValue" @update:modelValue="updateChannels" />
</template>
