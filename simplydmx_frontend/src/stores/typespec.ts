...
const activeTypeSpecs: Map<string, [number, Ref<ipc.DropdownOptionJSON[] | null>]> = reactive(
	new Map()
)

function fetchTypeSpecs() {
	activeTypeSpecs.forEach(([_, slot], typespecId) => {
		getTypeSpecOptions(typespecId).then(
			(options) => {
				slot.value = options
			},
...
export function useTypeSpecState(
	providerId:
		| string
		| null
		| Ref<string | null>
	...
	const providerIdResolver =
		typeof providerId === 'function'
			? providerId
			: () => {
	...
	if (
		(typeof providerId === 'object' || typeof providerId === 'function') &&
		providerId !== null
	) {
