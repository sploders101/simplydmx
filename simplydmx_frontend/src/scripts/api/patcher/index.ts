import { listen, patcher, VisualizationInfo } from '../ipc'
import type { SharablePatcherState } from '../ipc'

export async function listenForUpdates(
	mutableGetter: () => SharablePatcherState | null,
	setter: (state: SharablePatcherState) => void
): Promise<() => Promise<void>> {
	const patchUpdates = listen<void>('patcher.patch_updated', { type: 'None' }, () =>
		patcher.get_patcher_state().then(setter)
	)
	const libraryUpdates = listen<void>('patcher.new_fixture', { type: 'None' }, () =>
		patcher.get_patcher_state().then(setter)
	)
	...
	return () =>
		Promise.all([stopPatchUpdates(), stopLibraryUpdates(), stopVisualizationUpdates()]).then(
			() => {}
		)
}
