...
function mergeSubmasterData(src: ipc.SubmasterData, dest: ipc.SubmasterData): void {
	Object.entries(src).forEach(([fixtureId, fixtureValues]) => {
		const destFixtureValues = dest[fixtureId]
...
function setup(submasterId: ipc.Uuid) {
	const state =
		...
		(() => {
			...
			ipc.mixer.get_layer_contents(submasterId).then((data) => {
				if (data && state.data) {
...
export function useSubmasterData(submasterId: () => ipc.Uuid | null) {
	...
	watch(
		[submasterId, mounted],
		([newSubmasterId, newMounted], [oldSubmasterId, oldMounted]) => {
			if (
				newSubmasterId === null &&
				typeof oldSubmasterId === 'string' &&
				oldMounted &&
				newMounted
			) {
