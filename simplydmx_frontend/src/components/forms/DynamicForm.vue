...
import { FormItem, ExhaustiveFlag, exhaustiveMatch } from '@/scripts/api/ipc'
import { useTypeSpecState } from '@/stores/typespec'
import DynamicForm from './DynamicForm.vue'
import { checkDynamic, checkNumberValidation } from './helpers'
...
type _ExhaustiveFormCheck = ExhaustiveFlag<
	FormItem,
	{
		Dynamic: any
		Textbox: any
...
type SpecificItem<TypeKey extends string, T = Readonly<FormItem>> = T extends {
	[K in TypeKey]: any
}
	? T
	: never
...
const dropdownOptions = computed(() => {
	...
	return exhaustiveMatch(props.form.Dropdown.item_source, {
		...
		TypeSpec: () =>
			typeSpecOptions.value || [
				{ name: 'Loading...', value: props.formData[(props.form as any).Dropdown.id] }
			]
	})
})
</script>
...
<template>
	<template v-if="'Dynamic' in props.form && checkDynamic(props.formData, props.form.Dynamic[0])">
		<DynamicForm
			v-for="formItem in props.form.Dynamic[1]"
			:form="formItem"
	...
	<NumberInput
		...
		:error="
			!checkNumberValidation(
				props.formData[props.form.Number.id],
				props.form.Number.validation
			)
		"
	/>
	...
	<Dropdown
		...
		:error="
			dropdownOptions.some(
				(item) =>
					item.value ===
					props.formData[(props.form as SpecificItem<'Dropdown'>).Dropdown.id]
			)
				? null
				: 'Required'
		"
