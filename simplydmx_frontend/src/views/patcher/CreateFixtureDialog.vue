...
import { exhaustiveMatch, FormDescriptor, patcher } from '@/scripts/api/ipc'
...
const personalityOptions = computed(() => {
	...
	return Object.keys(state.value.library[fixtureToAdd.value].personalities).map<Option>(
		(key) => ({
			name: key,
			value: key
		})
...
watch(
	...
	() => {
		if (
			lightOptions.value.length &&
			!lightOptions.value.find((item) => item.value === selectedFixture.value)
		) {
...
watch(fixtureToAdd, (queryFixture) => {
	if (queryFixture) {
		patcher.get_creation_form(queryFixture).then((formResult) => {
...
<template>
	<Dialog :visible="props.visible" :show-close="false">
		...
		<template #footer>
			<Button @click="cancelAddingFixture()" subtle class="spaced"> Cancel </Button>
			<div class="spacer" />
			<Button @click="addFixture()" class="spaced"> Ok </Button>
		</template>
	</Dialog>
