	...
	const importResponse = await rpc.patcher.import_fixture({
		fixture_info: {
			...
			personalities: {
				...
				'16-bit': {
					available_channels: [
						'intensity16',
						...(color ? ['red16', 'green16', 'blue16', 'white16'] : []),
						...(panTilt ? ['pan16', 'tilt16'] : [])
					]
				}
			},
			output_driver: 'DMX',
			control_groups: [
				...
				...(color
					? [
						{
							name: null,
							channels: { RGBGroup: { red: 'red', green: 'green', blue: 'blue' } }
						},
						{
							name: null,
							channels: {
		...
		output_info: {
			personalities: {
				'8-bit': {
					dmx_channel_order: [
						...(color ? ['red', 'green', 'blue', 'white'] : ['intensity']),
						...(panTilt ? ['pan', 'tilt'] : [])
					]
				},
				'16-bit': {
					dmx_channel_order: [
						...(color ? ['red16', 'green16', 'blue16', 'white16'] : ['intensity']),
						...(panTilt ? ['pan16', 'tilt16'] : [])
					]
				}
			}
...
async function fullTestInvoked() {
	await importFixtureTest(
		'c205635c-037a-4e5c-8a68-59a8a86dae8f',
		'Generic RGBW Fixture',
		'RGBW',
		true,
		false
	)
	await importFixtureTest(
		'ce57c131-cc29-4088-84d2-1472b73f7d65',
		'Generic RGBWPT Fixture',
		'RGBWPT',
		true,
		true
	)
	await importFixtureTest(
		'bb5a93f8-4e26-422f-9ac8-5b9a15a9254d',
		'Generic Spotlight',
		'I',
		false,
		false
	...
	await importFixtureTest(
		'a93e6ba5-83ef-4faa-afd0-7b85dd12400b',
		'Generic Moving Spotlight',
		'IPT',
		false,
		true
	)
	...
	for (let i = 0; i < 17; i++) {
		let fixtureId = unwrap(
			await rpc.patcher.create_fixture(
				'c205635c-037a-4e5c-8a68-59a8a86dae8f',
				'8-bit',
				`LED ${i + 1}`,
				null,
...
<template>
	...
	>
		<Tabitem tab="test1">
			<p>
				This is currently my testing ground for all of the UI elements in SimplyDMX.
				Anything with functionality will be demonstrated here using a component. By
				isolating all user interactivity problems into a set of components, I get
				reusability and upgradability. For example, if I need a custom dropdown for a
				specific part of the app to augment its function, it will be defined first in the
				dropdown component, then used here, then used in the part of the app where I need
				it. By architecting user-interactivity in this way, I can easily add more input
				devices later like UI control via MIDI, or even more useful, touch for the eventual
				mobile build (This <strong><i>is</i></strong> coming; I've already done it, it just
				needs to be brought up to date with the new APIs and needs a bit more work to be
				stable. I'm also waiting on an official build of Wry that is declared as stable on
				mobile).
			</p>
			<p>{{ test }}</p>
			<p>{{ test2 }}</p>
		...
		<Tabitem tab="test2">
			<LargeSelect
				:options="[1, 2, 3, 4, 5].map((i) => ({ name: 'Test ' + i, value: i }))"
				v-model="selectValue"
				enableSearch
			/>
		</Tabitem>
	...
	<Dialog v-model:visible="dialogVisible">
		<template #header> Test Ran </template>

		The initialization test has been run. This test cannot be run again until the app has been
		restarted, as it may yield unexpected results. This will not be needed in the future as more
		UI elements are implemented.

		<template #footer>
			<Button @click="dialogVisible = false" subtle class="spaced"> Cancel </Button>
			<div class="spacer" />
			<Button @click="dialogVisible = false" class="spaced"> Ok </Button>
		</template>
	</Dialog>
</template>
