...
use self::interface::{DMXInterface, DMXShowSave};
...
pub async fn initialize(
	...
	save: Option<DMXShowSave>,
	patcher_interface: PatcherInterface,
) -> Result<DMXInterface, DMXInitializationError> {
	// Create plugin interface
	let output_context = if let Some(save) = save {
		DMXInterface::from_file(plugin_context.clone(), save)
	} else {
