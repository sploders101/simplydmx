...
<template>
	<Teleport to="body">
		<Transition name="sdmx-dialog-fade">
			<div
				v-if="props.visible"
				class="sdmx-dialog__modal"
				@click.stop="requestClose('modal')"
			>
				<div
					:class="'sdmx-dialog' + (props.dialogClass ? ' ' + props.dialogClass : '')"
					@click.stop
				>
					<div
						class="sdmx-dialog__header"
						v-if="$slots.header || props.showClose !== false"
					>
						<slot name="header" />
						<div class="spacer" />
						<Button
...
.sdmx-dialog-fade-enter-from,
.sdmx-dialog-fade-leave-to {
	opacity: 0;
}
.sdmx-dialog-fade-enter-active,
.sdmx-dialog-fade-leave-active {
