...
const personalityOptions = computed(() => {
	...
	return Object.keys(patcherState.value.library[fixture.fixture_id].personalities).map<Option>(
		(key) => ({
			name: key,
			value: key
		})
...
function initializeFixtureForm(queryFixture: string | null) {
	if (queryFixture) {
		...
		patcher.get_edit_form(queryFixture).then((formResult) => {
...
<template>
	...
	<div v-if="editsMade" class="fixture-editor__actions">
		<Button @click="cancelEditingFixture()" subtle class="spaced"> Cancel </Button>
		<div class="spacer" />
		<Button @click="editFixture()" class="spaced"> Ok </Button>
	</div>
